...
```{r, echo = TRUE, warning = FALSE, message = FALSE}
Data <- rolling_avg %>%
  mutate(avg_ws_m = avg_m_avg_ws, avg_rh_m = avg_m_avg_rh, avg_temp_m = avg_m_avg_temp, avg_dp_m = avg_m_avg_dp, avg_precip_m = avg_m_avg_precip)
...
plot(Data$Date, Data$avg_temp_m, type = "l", col = "blue",
     xlab = "Date", ylab = "Temperature *C", main = "Average Monthly Temperature")

plot(Data$Date, Data$avg_precip_m, type = "l", col = "blue",
...
variable_groups <- list(
  ...
  group_3 = c("avg_ws_m","avg_rh_m", "avg_temp_m", "avg_dp_m","avg_precip_m"),
  ...
  group_26 = c("avg_temp_m"),
  group_27 = c("avg_precip_m")
)
...
ccf_pairs <- list(
  c("Confirmed", "avg_temp_m"),
  c("Confirmed", "avg_precip_m"),
  c("Confirmed", "avg_temp_m"),
  c("Confirmed", "avg_temp_m"),
  c("Confirmed", "avg_temp_m"),
  c("Confirmed", "avg_temp_m"),
...
for (pair in ccf_pairs) {
  ccf(numeric_data[[pair[1]]], numeric_data[[pair[2]]], lag.max = 30, 
