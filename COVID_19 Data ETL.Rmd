...
We combine all daily Covid data records into a single data set ordered by State and Date.
```{r,echo= TRUE, eval=FALSE}
...
Income Dataset:
```{r, echo=FALSE}
example_6 <- read.csv("C:/Users/William Roche/Downloads/School/Portfolio/Covid Project/Data/poverty_income/Income.csv", check.names = FALSE)
head(example_6,5)
```

Additional Regressors Dataset:
```{r, echo=FALSE}
...
First use **left_join** by `State` and `Year`, then we join the rest of the data together before joining to original set by `State`,`Year`, and `Month` .
```{r, echo = TRUE, eval = TRUE}
...
Checking if any variables are correlated with each other, so that we can take out unnecessary variables.
...
num_cores <- detectCores() - 2  # Use all but 2 cores (for system stability)
...
Data2 <- Data2 %>% arrange(Date)  # Sort by date

start_year <- 2020
start_month <- 4
monthly_frequency <- 12

y_ts <- ts(Data2$y1, start = c(start_year, start_month), frequency = monthly_frequency)

xreg_matrix <- cbind(
  Data2$x1, Data2$x2, Data2$x3, Data2$x4, Data2$x5, Data2$x6, 
  Data2$x7, Data2$x8, Data2$x9, Data2$x10, Data2$x11, Data2$x12, 
  Data2$x13, Data2$x14, Data2$x15, Data2$x16
)

# Run SARIMA model with parallel processing
...
# Plot residuals to visually inspect patterns
checkresiduals(fit_sarima)

# Perform the Ljung-Box test for autocorrelation
Box.test(residuals(fit_sarima), lag = 20, type = "Ljung-Box")

```


```{r}
# Alternative SARIMA model (changing seasonal MA to 1 and adding non-seasonal MA)
alt_sarima <- auto.arima(y_ts, xreg = xreg_matrix, seasonal = TRUE, 
                         stepwise = FALSE, approximation = FALSE, 
                         d = 1, D = 0, max.p = 3, max.q = 3, 
                         max.P = 1, max.Q = 2)

# Compare AIC/BIC
cat("Original Model AIC:", AIC(fit_sarima), "BIC:", BIC(fit_sarima), "\n")
cat("Alternative Model AIC:", AIC(alt_sarima), "BIC:", BIC(alt_sarima), "\n")

dim(xreg_matrix)  # Find number of regressors used

future_xreg_matrix <- matrix(0, nrow=16, ncol=ncol(xreg_matrix))  # Adjust based on known values
colnames(future_xreg_matrix) <- colnames(xreg_matrix)  # Ensure column names match


# Generate forecasts for next 12 periods
future_forecast <- forecast(fit_sarima, xreg = future_xreg_matrix, h = 12)

# Plot the forecast
autoplot(future_forecast)

# Calculate accuracy of predictions
accuracy(future_forecast, actual_values)  # Replace actual_values with real future data if available


# Histogram of residuals
hist(residuals(fit_sarima), main = "Residual Distribution", xlab = "Residuals")

# ACF and PACF plots of residuals
acf(residuals(fit_sarima), main = "Residual ACF")
pacf(residuals(fit_sarima), main = "Residual PACF")


# Normality test (should be p > 0.05 for normal residuals)
shapiro.test(residuals(fit_sarima))

# Augmented Dickey-Fuller test (should be p < 0.05 for stationarity)
