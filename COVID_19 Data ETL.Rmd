...
data_dir <- "./Data/Average State Percipitation"
...
for (file in csv_files) {
  ...
  # Read CSV, skipping first 4 rows and without predefined headers
...
income_df <- income_df %>%
#  rename("State" = names(income_df)[0], "Year" = names(income_df)[1]) %>%
...
precip_m_df <- precip_m_df %>%
  mutate(State = as.character(State),     # Convert numeric → integer
         Year = as.integer(Year),
         Month = as.integer(Month),
         avg_percip_m = as.numeric(Value)) %>%
  select(State, Year, Month, avg_percip_m)
head(precip_m_df)
...
sapply(population_df,class) # year(2) char to int
#sapply(weather_df,class)
sapply(population2_df,class) # year(2) char to int
#sapply(traffic_df,class)
sapply(income_df,class) # poverty percent(3) & median income(4) from character to int
...
plot(Data_df$Date, Data_df$avg_precip, type = "l", col = "blue",
     xlab = "Date", ylab = "Rain (In.)", main = "Percipitaion")
...
```{r, warning = FALSE, message = FALSE}
acf(Data_df$avg_precip, main = "Autocorrelation Plot")
acf(Data_df$avg_dp, main = "Autocorrelation Plot")
acf(Data_df$avg_ws, main = "Autocorrelation Plot")
acf(Data_df$avg_temp, main = "Autocorrelation Plot")
acf(Data_df$Confirmed, main = "Autocorrelation Plot")
acf(Data_df$Deaths, main = "Autocorrelation Plot")
acf(Data_df$Recovered, main = "Autocorrelation Plot")
...
```{r, warning = FALSE, message = FALSE}
library(tseries)
adf.test(Data_df$avg_temp)  # Check stationarity
