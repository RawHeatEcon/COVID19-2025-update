...
library(dplyr)
Data_2 <- Data  %>%
  rename("y1" = Confirmed, "y2" = Deaths, "y3" = log_Confirmed, "x1" = pop_d, "x2"= avg_precip, "x3" = avg_ws, "x4" = avg_rh, "x5" = avg_temp, "x6" = avg_dp, "x7" = pop_infant, "x8" = pop_elder, "x9" = enplanements, "x10" = Poverty_Percent, "x11" = Median_Household_Income, "x12" = Days_Until_Lockdown, "x13" = Distance_To_Nearest_Epicenter, "x14" = Airport_Presence, "x15" = Beach_Presence, "x16" = Number_Of_State_Parks, "x17" = avg_ws_y, "x18" = avg_rh_y, "x19" = avg_temp_y, "x20" = avg_dp_y, "x21" = avg_precip_y, "x22" = predicted_cases, "x23" = predicted_log_cases, "x24" = log_pop_d, "x25" = log_Deaths, "x26" = log_avg_precip, "x27" = log_enplanements)

Data_3 <- Data_2 %>%
  select(State, y1, y2, y3, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27)
```
...
```{r}
model1 <- lm(y1~x1 + x2 + x3 + x4 + x5 + x6 + x7 + x8 + x9 + x10 + x11 + x12 + x13 + x14 + x15 + x16 + x17 + x18 + x19 + x20 + x21 + x22 + x23 + x24 + x25 + x26 + x27, Data_3)
